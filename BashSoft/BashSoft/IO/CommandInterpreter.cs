    ...
    public static class CommandInterpreter
    {
        ...
        private static void TryOrderAndTake(string input, string[] data)
        {
            if (data.Length == 5)
            {
                ...
                TryParseParametersForSortAndTake(takeCommand, takeQuantity, courseName, comparison);
            }
            else
            {
                DisplayInvalidCommandMessage(input);
        ...
        private static void TryParseParametersForSortAndTake(string takeCommand, string takeQuantity, string courseName, string comparison)
        {
            if (takeCommand == "take")
            {
                if (takeQuantity == "all")
        ...
        private static void TryShowWantedData(string input, string[] data)
        {
            if (data.Length==2)
            {
                string cousreName = data[1];
                StudentsRepository.GetAllStudentsFromCourse(cousreName);
            }
            else
            {
                if (data.Length==3)
                {
                    string cousreName = data[1];
                    string usertName = data[2];
                    StudentsRepository.GetSudentScoresFromCourse(cousreName, usertName);
                }
                else
                {
                    DisplayInvalidCommandMessage(input);
        ...
        private static void TryChangePathAbsolute(string input, string[] data)
        {
            if (data.Length == 2)
            {
                string newPath = data[1];
                IOManager.ChangeCurrentDirectoryAbsolute(newPath);
            }
            else
            {
                DisplayInvalidCommandMessage(input);
        ...
        private static void TryChangePathRelatively(string input, string[] data)
        {
            ...
            {
                string newPath = data[1];
                IOManager.ChangeCurrentDirectoryRalative(newPath);
            }
            else
            {
                DisplayInvalidCommandMessage(input);
